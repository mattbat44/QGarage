...
try:
    ...
    plugin_dir   = Path(cfg["plugin_dir"])   # e.g. .../python/plugins/qgarage
    ...
    _spec = importlib.util.spec_from_file_location("_qgarage_app_main", str(module_path))
    ...
    _safe_print(f"[QGarage] Running {app_meta.get('name', class_name)} ...\n", flush=True)
    try:
    ...
    _safe_print(f"\n[QGarage] [{status}] {result.get('message', '')}", flush=True)

except Exception as _fatal:
    # ── Catch-all: any error during setup, import, or config loading ──────
    _tb_text = _tb_mod.format_exc()
    _safe_print(f"\n[QGarage] FATAL ERROR during runner initialisation:\n{_tb_text}", flush=True)
